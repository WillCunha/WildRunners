import Carro from '@/components/Carro';
import { useCarSelection } from '@/context/CarContext';
import { carMaps } from '@/src/utils/carMaps';
import React, { useEffect, useRef, useState } from 'react';
import { LayoutAnimation, Platform, StyleSheet, Text, UIManager, View, useWindowDimensions } from 'react-native';

type CarKey = keyof typeof carMaps;

/* ================= CONFIGURAÇÕES DA FÍSICA E VELOCIDADE ================= */
const GRAVITY = 0.8;
const JUMP_FORCE = -15;
const PLAYER_SIZE = 50;

// Novas constantes de Corrida
const MAX_SPEED = 12;
const MIN_SPEED = 3; // Velocidade mínima para o jogo não parar totalmente
const IMPULSE_FORCE = 1.5;
const ACCELERATION = 0.3;
const FRICTION = 0.15;
const NITRO_SPEED = 22; // Velocidade durante o Nitro
const NITRO_DURATION = 60 * 3; // 3 segundos a 60fps

/* ================= CORES DISPONÍVEIS ================= */
const AVAILABLE_BOT_COLORS = [
  '#FF3B30', // Vermelho
  '#34C759', // Verde
  '#007AFF', // Azul
  '#FFCC00', // Amarelo
  '#FF9500', // Laranja
  '#AF52DE', // Roxo
  '#1C1C1E', // Preto
  '#F2F2F7', // Branco
];

export default function Mapa() {

  const { width: SCREEN_WIDTH, height: SCREEN_HEIGHT } = useWindowDimensions();

  if (Platform.OS === 'android' && UIManager.setLayoutAnimationEnabledExperimental) {
    UIManager.setLayoutAnimationEnabledExperimental(true);
  }

  const { selectedCar, selectedColor } = useCarSelection();

  const BASE_PLAYER_X = SCREEN_WIDTH * 0.4;
  const GAP_BETWEEN_RACERS = 60;
  const TOTAL_RACERS = 6;

  type CardEffect = 'swap' | 'heavy_gravity' | 'invert_controls' | 'blind' | 'panic' | 'ghost' | 'score_boost';

  type Block = {
    id: number;
    type: 'flat' | 'ramp';
    x: number;
    width: number;
    y?: number; // Usado para flat
    startY?: number; // Usado para ramp
    endY?: number; // Usado para ramp
  };

  type Bot = {
    id: string;
    name: string;
    deck: CardEffect[]; // Seu tipo de deck
    x: number;
    y: number;
    speed: number;
    targetSpeed: number;
    skin: string;
    isCrouching: boolean;
    velocity: number;
    score: number;
    thinkTimer: number;
    status: typeof defaultStatus;
    activeEffectsTimers: Partial<Record<CardEffect, number>>;
    // *** NOVOS CAMPOS *** //
    carType: CarKey;      // Tipo de carro (fusca, kombi, etc)
    carColor: string;     // Cor do carro
  };

  const BOT_NAMES = [
    'Relâmpago', 'Marquinhos', 'Trovão', 'Faísca', 'Brisa',
    'Ventania', 'Cometa', 'Nitro', 'Sombra', 'Turbina', 'Rex'
  ];

  const CARD_CATEGORIES = {
    HEAVY_ATTACK: ['swap', 'heavy_gravity', 'blind'],
    LIGHT_ATTACK: ['invert_controls', 'panic'],
    DEFENSE_BUFF: ['ghost', 'score_boost']
  };

  const COOLDOWNS = { HEAVY: 60 * 15, LIGHT: 60 * 8, DEFENSE: 60 * 12 };

  const defaultStatus = { gravityMultiplier: 1, controlsInverted: false, isBlind: false, isPanicking: false, isGhost: false, scoreMultiplier: 1 };

  const playerStatus = useRef({ ...defaultStatus });
  const activeEffectsTimers = useRef<Partial<Record<CardEffect, number>>>({});

  const isCrouchingRef = useRef(false);
  const [isCrouching, setIsCrouching] = useState(false);

  const playerSpeed = useRef(MIN_SPEED);
  const nitroCharge = useRef(0); // Vai de 0 a 100
  const isNitroActive = useRef(false);
  const nitroTimer = useRef(0);

  const y = useRef(SCREEN_HEIGHT / 2);
  const playerXRef = useRef(BASE_PLAYER_X);
  const velocity = useRef(0);
  const isGrounded = useRef(false);
  const gameTime = useRef(0);

  const blocksRef = useRef<Block[]>([
    { id: 1, type: 'flat', x: 0, y: SCREEN_HEIGHT - 100, width: SCREEN_WIDTH * 1.5 }
  ]);

  const getRandomColor = () => {
    return AVAILABLE_BOT_COLORS[Math.floor(Math.random() * AVAILABLE_BOT_COLORS.length)];
  };

  const getRandomCarType = (): CarKey => {
    const carKeys: CarKey[] = Object.keys(carMaps) as CarKey[];
    return carKeys[Math.floor(Math.random() * carKeys.length)];
  };
  const botsRef = useRef([
    { id: 'bot1', name: getRandomName(), deck: generateRandomDeck(), x: 0, y: SCREEN_HEIGHT / 2, speed: 0, targetSpeed: MAX_SPEED, skin: 'default', isCrouching: false, velocity: 0, score: 0, deck: [{ effect: 'swap', currentCooldown: 0, baseCooldown: COOLDOWNS.HEAVY }, { effect: 'blind', currentCooldown: 0, baseCooldown: COOLDOWNS.HEAVY }], thinkTimer: 0, status: { ...defaultStatus }, activeEffectsTimers: {}, carType: getRandomCarType(), carColor: getRandomColor() },
    { id: 'bot2', name: getRandomName(), deck: generateRandomDeck(), x: 0, y: SCREEN_HEIGHT / 2, speed: 0, targetSpeed: MAX_SPEED, skin: 'gangster', isCrouching: false, velocity: 0, score: 0, deck: [{ effect: 'swap', currentCooldown: 0, baseCooldown: COOLDOWNS.HEAVY }, { effect: 'panic', currentCooldown: 0, baseCooldown: COOLDOWNS.LIGHT }], thinkTimer: 0, status: { ...defaultStatus }, activeEffectsTimers: {}, carType: getRandomCarType(), carColor: getRandomColor() },
    { id: 'bot3', name: getRandomName(), deck: generateRandomDeck(), x: 0, y: SCREEN_HEIGHT / 2, speed: 0, targetSpeed: MAX_SPEED, skin: 'ninja', isCrouching: false, velocity: 0, score: 0, deck: [{ effect: 'swap', currentCooldown: 0, baseCooldown: COOLDOWNS.HEAVY }, { effect: 'panic', currentCooldown: 0, baseCooldown: COOLDOWNS.LIGHT }], thinkTimer: 0, status: { ...defaultStatus }, activeEffectsTimers: {}, carType: getRandomCarType(), carColor: getRandomColor() },
    { id: 'bot4', name: getRandomName(), deck: generateRandomDeck(), x: 0, y: SCREEN_HEIGHT / 2, speed: 0, targetSpeed: MAX_SPEED, skin: 'pirate', isCrouching: false, velocity: 0, score: 0, deck: [{ effect: 'swap', currentCooldown: 0, baseCooldown: COOLDOWNS.HEAVY }, { effect: 'panic', currentCooldown: 0, baseCooldown: COOLDOWNS.LIGHT }], thinkTimer: 0, status: { ...defaultStatus }, activeEffectsTimers: {}, carType: getRandomCarType(), carColor: getRandomColor() },
    { id: 'bot5', name: getRandomName(), deck: generateRandomDeck(), x: 0, y: SCREEN_HEIGHT / 2, speed: 0, targetSpeed: MAX_SPEED, skin: 'surfer', isCrouching: false, velocity: 0, score: 0, deck: [{ effect: 'swap', currentCooldown: 0, baseCooldown: COOLDOWNS.HEAVY }, { effect: 'panic', currentCooldown: 0, baseCooldown: COOLDOWNS.LIGHT }], thinkTimer: 0, status: { ...defaultStatus }, activeEffectsTimers: {}, carType: getRandomCarType(), carColor: getRandomColor() },
  ]);

  const [nitroPercent, setNitroPercent] = useState(0);
  const [isNitroReady, setIsNitroReady] = useState(false);

  const [playerY, setPlayerY] = useState(y.current);
  const [playerX, setPlayerX] = useState(playerXRef.current);

  const [bots, setBots] = useState(botsRef.current);
  const [started, setStarted] = useState(false);
  const [gameOver, setGameOver] = useState(false);
  const [score, setScore] = useState(0);

  const [countdownText, setCountdownText] = useState<string | null>(null);
  const isCountingRef = useRef(false);

  const [leaderboard, setLeaderboard] = useState<{ id: string, name: string }[]>([]);
  const lastOrderRef = useRef('');

  const [blocks, setBlocks] = useState(blocksRef.current);
  const [isBlindActive, setIsBlindActive] = useState(false);
  const [isGhostActive, setIsGhostActive] = useState(false);

  /* ================= SORTEIA AS POSIÇÕES ================= */
  const setupPositions = () => {
    const positions = Array.from({ length: TOTAL_RACERS }, (_, i) => BASE_PLAYER_X - (i * GAP_BETWEEN_RACERS));

    for (let i = positions.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [positions[i], positions[j]] = [positions[j], positions[i]];
    }

    playerXRef.current = positions[0];
    setPlayerX(positions[0]);

    const newBots = [...botsRef.current];
    for (let i = 0; i < 5; i++) {
      newBots[i].x = positions[i + 1];
      newBots[i].y = SCREEN_HEIGHT / 2;
      newBots[i].velocity = 0;
      newBots[i].speed = MIN_SPEED; // Bots começam devagar
      newBots[i].status = { ...defaultStatus };
      newBots[i].activeEffectsTimers = {};
      newBots[i].deck = generateRandomDeck();
      newBots[i].deck.forEach(c => c.currentCooldown = 0);
      newBots[i].carType = getRandomCarType();
      newBots[i].carColor = getRandomColor();
    }
    botsRef.current = newBots;
    setBots(newBots);
  };

  /* ================= PREPARA O GAME ================= */
  useEffect(() => {
    if (!started && !isCountingRef.current) {
      y.current = SCREEN_HEIGHT / 2;
      setPlayerY(y.current);
      blocksRef.current = [{ id: 1, type: 'flat', x: 0, y: SCREEN_HEIGHT - 100, width: SCREEN_WIDTH * 1.5 }];
      setBlocks(blocksRef.current);
      setupPositions();
    }
  }, [SCREEN_HEIGHT, SCREEN_WIDTH]);

  /* ================= GAME LOOP ================= */
  useEffect(() => {
    if (!started || gameOver) return;

    const loop = setInterval(() => {
      gameTime.current += 1;

      // --- 1. LÓGICA DE VELOCIDADE DO JOGADOR ---
      if (isNitroActive.current) {
        playerSpeed.current = NITRO_SPEED;
        nitroTimer.current -= 1;

        if (nitroTimer.current <= 0) {
          isNitroActive.current = false;
          nitroCharge.current = 0;
          setIsNitroReady(false);
          setNitroPercent(0);
        }
      } else {
        playerSpeed.current = Math.max(playerSpeed.current - FRICTION, MIN_SPEED);
      }

      const dynamicSpeed = playerSpeed.current;

      // --- 2. LÓGICA DE VÁCUO (DRAFTING) PARA O NITRO ---
      let isDrafting = false;
      botsRef.current.forEach(bot => {
        const distanceToBot = bot.x - playerXRef.current;
        // Se o bot está na frente, perto e na mesma plataforma (Y similar)
        if (distanceToBot > 10 && distanceToBot < 120 && Math.abs(bot.y - y.current) < 30) {
          isDrafting = true;
        }
      });

      if (isDrafting && !isNitroActive.current && nitroCharge.current < 100) {
        nitroCharge.current += 0.8; // Carrega a barra (aprox 2 segundos no vácuo)
        if (nitroCharge.current >= 100) {
          nitroCharge.current = 100;
          if (!isNitroReady) setIsNitroReady(true);
        }
        if (gameTime.current % 5 === 0) setNitroPercent(nitroCharge.current);
      } else if (!isDrafting && !isNitroActive.current && nitroCharge.current > 0) {
        // Se sair do vácuo, perde carga aos poucos (Opcional, deixa o jogo mais hardcore)
        nitroCharge.current = Math.max(nitroCharge.current - 0.2, 0);
        if (nitroCharge.current < 100 && isNitroReady) setIsNitroReady(false);
        if (gameTime.current % 5 === 0) setNitroPercent(nitroCharge.current);
      }


      // --- 3. ATUALIZAÇÃO DO PLACAR ---
      if (gameTime.current % 10 === 0) {
        const allRacers = [
          { id: 'player', name: 'Você (P1)', x: playerXRef.current },
          ...botsRef.current.map(b => ({ id: b.id, name: b.name, x: b.x }))
        ].sort((a, b) => b.x - a.x);

        const currentOrder = allRacers.map(r => r.id).join(',');

        if (currentOrder !== lastOrderRef.current) {
          LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
          setLeaderboard(allRacers.map(r => ({ id: r.id, name: r.name })));
          lastOrderRef.current = currentOrder;
        }
      }

      // Efeitos de Status (Timers)
      for (const [effect, timeLeft] of Object.entries(activeEffectsTimers.current)) {
        if (timeLeft && timeLeft > 0) {
          activeEffectsTimers.current[effect as CardEffect] = timeLeft - 1;
          if (timeLeft - 1 === 0) {
            switch (effect) {
              case 'heavy_gravity': playerStatus.current.gravityMultiplier = 1; break;
              case 'invert_controls': playerStatus.current.controlsInverted = false; break;
              case 'blind': playerStatus.current.isBlind = false; setIsBlindActive(false); break;
              case 'panic': playerStatus.current.isPanicking = false; break;
              case 'ghost': playerStatus.current.isGhost = false; setIsGhostActive(false); break;
              case 'score_boost': playerStatus.current.scoreMultiplier = 1; break;
            }
          }
        }
      }

      if (playerStatus.current.isPanicking && isGrounded.current && gameTime.current % 30 === 0) {
        velocity.current = JUMP_FORCE;
        isGrounded.current = false;
      }

      const currentGravity = GRAVITY * playerStatus.current.gravityMultiplier;
      velocity.current += currentGravity;
      y.current += velocity.current;

      // --- 4. GERAÇÃO E MOVIMENTO DOS BLOCOS ---
      let updatedBlocks = blocksRef.current.map(block => ({
        ...block,
        x: block.x - dynamicSpeed
      }));

      const lastBlock = updatedBlocks[updatedBlocks.length - 1];

      if (lastBlock && lastBlock.x + lastBlock.width < SCREEN_WIDTH) {
        const minWidth = 150;
        const maxWidth = 350;
        const newWidth = Math.random() * (maxWidth - minWidth) + minWidth;

        // Define se sobe, desce ou vai reto
        let direction = Math.floor(Math.random() * 3) - 1;

        // Pega o Y final do último bloco para conectar o terreno suavemente
        const currentY = lastBlock.type === 'ramp' ? lastBlock.endY! : lastBlock.y;
        let nextY = currentY + (direction * 80);

        if (nextY < 150) nextY = 150;
        if (nextY > SCREEN_HEIGHT - 100) nextY = SCREEN_HEIGHT - 100;

        if (nextY < currentY) {
          // O próximo é mais alto: Cria uma Rampa (sem buraco)
          updatedBlocks.push({
            id: Date.now(),
            type: 'ramp',
            x: lastBlock.x + lastBlock.width,
            startY: currentY,
            endY: nextY,
            width: 250, // Rampas tem largura fixa para manter o ângulo
          });
        } else {
          // Reto ou Desce: Cria um bloco flat com buraco para pulo
          let gap = Math.random() * 120 + 80;
          updatedBlocks.push({
            id: Date.now(),
            type: 'flat',
            x: lastBlock.x + lastBlock.width + gap,
            y: nextY,
            width: newWidth,
          });
        }
      }

      blocksRef.current = updatedBlocks.filter(block => block.x + block.width > -100);
      // --- 5. INTELIGÊNCIA DE CORRIDA DOS BOTS ---
      botsRef.current.forEach(bot => {
        // IA para gerenciar a própria velocidade
        let targetSpeed = MAX_SPEED * (0.8 + Math.random() * 0.2); // Bots correm a 80-100% do máximo

        // Se o bot estiver muito atrás do player, ele tenta correr mais rápido para alcançar
        if (bot.x < playerXRef.current - 150) {
          targetSpeed = MAX_SPEED * 1.1;
        }

        if (bot.speed < targetSpeed) bot.speed += ACCELERATION * 0.8;
        if (bot.speed > targetSpeed) bot.speed -= FRICTION;

        // O movimento do bot na tela é a diferença entre a velocidade dele e a SUA velocidade
        bot.x += (bot.speed - dynamicSpeed);

        // Limita para os bots não irem infinitamente para frente ou para trás da tela
        if (bot.x > SCREEN_WIDTH - 50) bot.x = SCREEN_WIDTH - 50;
        if (bot.x < -100) bot.x = -100;

        // Física básica do bot
        bot.deck.forEach(card => {
          if (card.currentCooldown > 0) card.currentCooldown -= 1;
        });

        for (const [effect, timeLeft] of Object.entries(bot.activeEffectsTimers)) {
          if (timeLeft && (timeLeft as number) > 0) {
            bot.activeEffectsTimers[effect as CardEffect] = (timeLeft as number) - 1;
            if ((timeLeft as number) - 1 === 0) {
              switch (effect) {
                case 'heavy_gravity': bot.status.gravityMultiplier = 1; break;
                case 'panic': bot.status.isPanicking = false; break;
              }
            }
          }
        }

        if (bot.status.isPanicking && bot.velocity === 0 && gameTime.current % 30 === 0) {
          bot.velocity = JUMP_FORCE;
        }

        const currentBotGravity = GRAVITY * bot.status.gravityMultiplier;
        bot.velocity += currentBotGravity;
        bot.y += bot.velocity;

        const botFootY = bot.y + PLAYER_SIZE;
        const botRightX = bot.x + PLAYER_SIZE;

        for (let block of blocksRef.current) {
          const isWithinX = (botRightX > block.x) && (bot.x < block.x + block.width);
          const previousFootY = botFootY - bot.velocity;

          if (isWithinX) {
            if (bot.velocity >= 0 && previousFootY <= block.y && botFootY >= block.y) {
              bot.y = block.y - PLAYER_SIZE;
              bot.velocity = 0;

              const distanceToEnd = (block.x + block.width) - bot.x;
              let reactionDistance = 80 + (Math.random() * 40);
              let willFail = false;

              if (bot.status.isBlind) {
                if (Math.random() < 0.4) willFail = true;
                reactionDistance = 30 + (Math.random() * 20);
              }

              if (!willFail && distanceToEnd < reactionDistance) {
                bot.velocity = JUMP_FORCE;
              }
              break;
            }
          }
        }
      });
      // Filtra bots mortos
      botsRef.current = botsRef.current.filter(bot => bot.y <= SCREEN_HEIGHT + 100);

      if (gameTime.current % 30 === 0) {
        processBotsAI();
      }

      // --- 6. COLISÕES DO PLAYER ---
      let landedOnBlock = false;
      const playerFootY = y.current + PLAYER_SIZE;
      // Usamos o centro do personagem para não "flutuar" nas bordas da rampa
      const playerCenterX = playerXRef.current + (PLAYER_SIZE / 2);

      for (let block of blocksRef.current) {
        const isWithinX = (playerCenterX > block.x) && (playerCenterX < block.x + block.width);
        const previousFootY = playerFootY - velocity.current;

        if (isWithinX) {
          if (block.type === 'flat') {
            if (velocity.current >= 0 && previousFootY <= block.y && playerFootY >= block.y) {
              y.current = block.y - PLAYER_SIZE;
              velocity.current = 0;
              landedOnBlock = true;
              break;
            }
          } else if (block.type === 'ramp') {
            // Calcula qual a porcentagem da rampa o carrinho já andou
            const progress = (playerCenterX - block.x) / block.width;
            // Descobre a altura exata do chão naquele exato pixel X
            const currentRampY = block.startY! + ((block.endY! - block.startY!) * progress);

            if (velocity.current >= 0 && playerFootY >= currentRampY - 15) { // -15px dá uma tolerância visual gostosa
              y.current = currentRampY - PLAYER_SIZE;
              velocity.current = 0;
              landedOnBlock = true;
              break;
            }
          }
        }
      }

      isGrounded.current = landedOnBlock;
      if (y.current > SCREEN_HEIGHT + 100) {
        setGameOver(true);
      }

      // Atualiza a distância percorrida baseada na velocidade real do player
      if (gameTime.current % 10 === 0) setScore(s => s + Math.floor(playerSpeed.current / 3));

      setPlayerY(y.current);
      setPlayerX(playerXRef.current);
      setBlocks(blocksRef.current);
      setBots([...botsRef.current])
    }, 16);

    return () => clearInterval(loop);
  }, [started, gameOver, SCREEN_WIDTH, SCREEN_HEIGHT]);

  /* ================= GERA NOME PARA OS BOTS ================= */
  function getRandomName() {
    return BOT_NAMES[Math.floor(Math.random() * BOT_NAMES.length)];
  }

  /* ================= SORTEIA O DECK DOS BOTS ================= */
  function generateRandomDeck() {
    const allEffects: CardEffect[] = ['swap', 'heavy_gravity', 'invert_controls', 'blind', 'panic', 'ghost', 'score_boost'];
    const shuffled = allEffects.sort(() => 0.5 - Math.random());

    return [
      {
        effect: shuffled[0],
        currentCooldown: 60 * 3 + Math.floor(Math.random() * 120), // Delay inicial aleatório (3 a 5 segs) para não atirarem na largada
        baseCooldown: COOLDOWNS.HEAVY // Você pode aprimorar isso mapeando o cooldown real por categoria depois
      },
      {
        effect: shuffled[1],
        currentCooldown: 60 * 3 + Math.floor(Math.random() * 120),
        baseCooldown: COOLDOWNS.LIGHT
      }
    ];
  }

  /* ================= HANDLES IMPULSE ================= */
  function handleAddImpulse() {
    if (gameOver || isNitroActive.current) return;

    playerSpeed.current = Math.min(playerSpeed.current + IMPULSE_FORCE, MAX_SPEED);
  }

  /* ================= IA DOS BOTS ================= */
  function processBotsAI() {
    const allRacers = [
      { id: 'player', x: playerXRef.current, isPlayer: true },
      ...botsRef.current.map(b => ({ id: b.id, x: b.x, isPlayer: false }))
    ].sort((a, b) => b.x - a.x);

    botsRef.current.forEach(bot => {
      if (bot.thinkTimer > 0) {
        bot.thinkTimer--;
        return;
      }

      const availableCards = bot.deck.filter(card => card.currentCooldown <= 0);
      if (availableCards.length === 0) return;

      const myRank = allRacers.findIndex(r => r.id === bot.id);

      // Escolhe uma carta disponível aleatoriamente, em vez de sempre priorizar o swap
      const chosenCard = availableCards[Math.floor(Math.random() * availableCards.length)];
      let target = bot.id;

      if (CARD_CATEGORIES.DEFENSE_BUFF.includes(chosenCard.effect as any)) {
        target = bot.id; // Magias de defesa sempre em si mesmo
      } else {
        // É um ataque. Quem vamos focar?
        const opponentsAhead = allRacers.slice(0, myRank);

        if (opponentsAhead.length > 0) {
          if (chosenCard.effect === 'swap') {
            // Swap: 60% de chance de mirar no 1º colocado, 40% de mirar em alguém aleatório à frente
            if (Math.random() > 0.4) {
              target = opponentsAhead[0].id;
            } else {
              target = opponentsAhead[Math.floor(Math.random() * opponentsAhead.length)].id;
            }
          } else {
            // Outros ataques: 70% de chance de atacar quem está imediatamente na frente, 30% aleatório
            if (Math.random() > 0.3) {
              target = opponentsAhead[opponentsAhead.length - 1].id; // O cara logo na frente
            } else {
              target = opponentsAhead[Math.floor(Math.random() * opponentsAhead.length)].id;
            }
          }
        } else {
          // Sou o 1º colocado! Ataco quem está imediatamente atrás de mim
          const opponentsBehind = allRacers.slice(myRank + 1);
          if (opponentsBehind.length > 0) {
            target = opponentsBehind[0].id;
          } else {
            return; // Não tem ninguém para atacar
          }
        }
      }

      // Validação final de Swap (não gasta se o alvo já estiver atrás de alguma forma)
      if (chosenCard.effect === 'swap') {
        const targetRacer = allRacers.find(r => r.id === target);
        if (targetRacer && targetRacer.x <= bot.x) {
          bot.thinkTimer = 30; // Espera e pensa de novo daqui a meio segundo
          return;
        }
      }

      applyCardEffect(chosenCard.effect as CardEffect, target, bot.id);
      chosenCard.currentCooldown = chosenCard.baseCooldown;
      // Adiciona um tempo de reação após usar um poder (1 a 3 segundos de folga)
      bot.thinkTimer = 60 + Math.floor(Math.random() * 120);
    });
  }

  /* ================= EFEITO DAS CARTAS================= */
  function applyCardEffect(effect: CardEffect, targetId: string, sourceId: string) {
    const DURATION = 60 * 4;

    if (effect === 'swap') {
      const sourceBot = botsRef.current.find(b => b.id === sourceId);

      if (sourceId === 'player') {
        const targetBot = botsRef.current.find(b => b.id === targetId);
        if (targetBot) {
          const tempY = y.current;
          const tempX = playerXRef.current

          y.current = targetBot.y;
          playerXRef.current = targetBot.x;

          targetBot.y = tempY;
          targetBot.x = tempX;
        }
      }
      else if (sourceBot) {
        if (targetId === 'player') {
          const tempY = y.current;
          const tempX = playerXRef.current;

          y.current = sourceBot.y;
          playerXRef.current = sourceBot.x;

          sourceBot.y = tempY;
          sourceBot.x = tempX;
        } else {
          const targetBot = botsRef.current.find(b => b.id === targetId);
          if (targetBot) {
            const tempY = targetBot.y;
            const tempX = targetBot.x

            targetBot.y = sourceBot.y;
            sourceBot.y = tempY;

            targetBot.x = sourceBot.x;
            sourceBot.x = tempX;
          }
        }
      }
      return;
    }

    if (targetId === 'player') {
      activeEffectsTimers.current[effect] = DURATION;
      switch (effect) {
        case 'heavy_gravity': playerStatus.current.gravityMultiplier = 2; break;
        case 'invert_controls': playerStatus.current.controlsInverted = true; break;
        case 'blind': playerStatus.current.isBlind = true; setIsBlindActive(true); break;
        case 'panic': playerStatus.current.isPanicking = true; break;
        case 'ghost': playerStatus.current.isGhost = true; setIsGhostActive(true); break;
        case 'score_boost': playerStatus.current.scoreMultiplier = 2; break;
      }
    } else if (targetId !== 'player') {
      const targetBot = botsRef.current.find(b => b.id === targetId);
      if (targetBot) {
        targetBot.activeEffectsTimers[effect] = DURATION;
        switch (effect) {
          case 'heavy_gravity': targetBot.status.gravityMultiplier = 2; break;
          case 'panic': targetBot.status.isPanicking = true; break;
          case 'blind': targetBot.status.isBlind = true; targetBot.activeEffectsTimers.blind = DURATION;
        }
      }
    }
  }

  /* ================= AGUARDA ANTES DE COMEÇAR A PARTIDA ================= */
  const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

  /* ================= CONTAGEM REGRESSIVA ================= */
  const startRaceSequence = async () => {
    if (isCountingRef.current) return;
    isCountingRef.current = true;

    setCountdownText('PREPARANDO...');
    await sleep(2000);
    setCountdownText('3');
    await sleep(1000);
    setCountdownText('2');
    await sleep(1000);
    setCountdownText('1');
    await sleep(1000);
    setCountdownText('JÁ!');
    await sleep(500);

    setCountdownText(null);
    setStarted(true);
    isCountingRef.current = false;
  };

  /* ================= CONTROLA OS PULOS ================= */
  function handleJump() {
    if (gameOver) {
      setGameOver(false);
      setStarted(false);
      setScore(0);
      gameTime.current = 0;
      nitroCharge.current = 0;
      setNitroPercent(0);
      setIsNitroReady(false);
      isNitroActive.current = false;

      y.current = SCREEN_HEIGHT / 2;
      velocity.current = 0;
      blocksRef.current = [{ id: Date.now(), type: 'flat', x: 0, y: SCREEN_HEIGHT - 100, width: SCREEN_WIDTH * 1.5 }];
      setBlocks(blocksRef.current);
      setupPositions();
      return;
    }

    if (playerStatus.current.controlsInverted) {
      isCrouchingRef.current = true;
      setIsCrouching(true);
      setTimeout(() => {
        isCrouchingRef.current = false;
        setIsCrouching(false);
      }, 500);
      return;
    }

    if (isGrounded.current) {
      velocity.current = JUMP_FORCE;
      isGrounded.current = false;
    }
  }

  /* ================= ATIVA O NITRO ================= */
  function handleActivateNitro() {
    if (isNitroReady && !isNitroActive.current) {
      isNitroActive.current = true;
      nitroTimer.current = NITRO_DURATION;
      setIsNitroReady(false);
    }
  }

  /* ================= RENDER ================= */
  return (
    <View style={styles.container}>
      {/* Área de Pulo agora pega a tela inteira (embaixo dos botões) */}
      <View onTouchStart={startRaceSequence} />

      <View style={styles.leaderboardContainer} pointerEvents="none">
        <Text style={styles.leaderboardTitle}>RANKING</Text>
        {leaderboard.map((racer, index) => (
          <View
            key={racer.id}
            style={[
              styles.leaderboardItem,
              racer.id === 'player' && styles.leaderboardItemPlayer
            ]}
          >
            <Text style={styles.leaderboardRank}>{index + 1}º</Text>
            <Text style={styles.leaderboardName} numberOfLines={1}>{racer.name}</Text>
          </View>
        ))}
      </View>

      {/* --- NOVA BARRA DE NITRO SUPERIOR --- */}
      <View style={styles.nitroBarContainer}>
        <View style={[styles.nitroBarFill, { width: `${nitroPercent}%`, backgroundColor: isNitroReady ? '#00FFFF' : '#FFD700' }]} />
        <Text style={styles.nitroBarText}>VÁCUO</Text>
      </View>

      <View style={styles.hud}>
        <Text style={styles.scoreText}>{score}m</Text>
      </View>

      {blocks.map((b) => {
        if (b.type === 'flat') {
          return (
            <View
              key={`flat-${b.id}`} // Usando b.id
              style={[styles.block, { left: b.x, top: b.y, width: b.width }]}
            />
          );
        } else if (b.type === 'ramp') {
          // Cálculos trigonométricos para rotacionar a rampa
          const dy = b.endY! - b.startY!;
          const dx = b.width;
          const length = Math.sqrt(dx * dx + dy * dy);
          const angle = Math.atan2(dy, dx) * (180 / Math.PI);

          // Calcula o centro da rampa
          const midX = b.x + dx / 2;
          const midY = (b.startY! + b.endY!) / 2;

          return (
            <View
              key={`ramp-${b.id}`} // Usando b.id
              style={[
                styles.block,
                {
                  // Posiciona pelo centro geométrico
                  left: midX - length / 2,
                  top: midY,
                  width: length,
                  height: 2000,
                  transform: [{ rotate: `${angle}deg` }]
                }
              ]}
            />
          );
        }
      })}
      {/* {bots.map((bot) => (
        <View
          key={bot.id}
          style={{
            position: 'absolute',
            top: bot.y,
            left: bot.x,
            zIndex: 4
          }}
        >
          <Carro
            carType={bot.carType}
            carColor={bot.carColor}
            speed={bot.speed}
            skin={bot.skin}
          />
        </View>
      ))
      } */}

      <View
        style={{
          position: 'absolute',
          zIndex: 5,
          opacity: isGhostActive ? 0.4 : 1,
          left: playerX,
          top: playerY,
        }}
      >
        <Carro
          carType={selectedCar}
          carColor={selectedColor}
          speed={playerSpeed.current}
          skin="default"
        />
      </View>

      {isBlindActive && <View style={styles.blindEffect} pointerEvents="none" />}

      <View
        style={styles.jumpArea}
        onStartShouldSetResponder={() => true}
        onResponderGrant={handleJump}
      >
        <Text style={styles.throttleBtnText}>Pular</Text>
      </View>
      {
        started && !gameOver && (
          <View style={styles.drivingControls}>
            {isNitroReady && (
              <View style={styles.nitroBtn} onTouchStart={handleActivateNitro}>
                <Text style={styles.nitroBtnText}>NITRO</Text>
              </View>
            )}

            <View
              style={styles.throttleBtn}
              // Usamos onTouchStart para disparar o impulso a cada toque individual
              onTouchStart={handleAddImpulse}
            >
              <Text style={styles.throttleBtnText}>Acelerar</Text>
            </View>
          </View>
        )
      }

      {/* --- OVERLAYS DE ESTADO --- */}
      {
        !started && !gameOver && !countdownText && (
          <View style={styles.overlay} pointerEvents="none">
            <Text style={styles.titleText}>TOQUE PARA CORRER</Text>
          </View>
        )
      }

      {
        countdownText && (
          <View style={styles.overlay} pointerEvents="none">
            <Text style={styles.countdownText}>{countdownText}</Text>
          </View>
        )
      }

      {
        gameOver && (
          <View style={styles.overlay} pointerEvents="none">
            <Text style={styles.titleText}>CAIU!</Text>
            <Text style={{ color: '#fff', fontSize: 20 }}>Você correu {score}m</Text>
            <Text style={{ color: '#aaa', marginTop: 20 }}>Toque para tentar de novo</Text>
          </View>
        )
      }
    </View >
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#1E1E2E',
    overflow: 'hidden'
  },
  hud: {
    position: 'absolute',
    top: 60, // Ajustado para não bater na barra de nitro
    right: 50,
    zIndex: 10
  },
  scoreText: {
    fontSize: 32,
    fontWeight: '900',
    color: '#FFF',
    fontStyle: 'italic'
  },
  leaderboardContainer: {
    position: 'absolute',
    top: 60,
    left: 20,
    backgroundColor: 'rgba(0, 0, 0, 0.5)',
    padding: 10,
    borderRadius: 10,
    zIndex: 10,
    width: 150,
  },
  leaderboardTitle: {
    color: '#FFD700',
    fontWeight: '900',
    fontStyle: 'italic',
    marginBottom: 5,
    textAlign: 'center',
    fontSize: 12,
  },
  leaderboardItem: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: 'rgba(255, 255, 255, 0.1)',
    marginBottom: 4,
    paddingVertical: 4,
    paddingHorizontal: 8,
    borderRadius: 5,
  },
  leaderboardItemPlayer: {
    backgroundColor: 'rgba(0, 208, 132, 0.4)',
    borderWidth: 1,
    borderColor: '#00D084',
  },
  leaderboardRank: {
    color: '#FFF',
    fontWeight: 'bold',
    width: 25,
    fontSize: 12,
  },
  leaderboardName: {
    color: '#FFF',
    fontSize: 12,
    flex: 1,
  },

  /* ESTILOS DA BARRA DE NITRO */
  nitroBarContainer: {
    position: 'absolute',
    top: 20,
    alignSelf: 'center',
    width: '50%',
    height: 20,
    backgroundColor: 'rgba(0,0,0,0.5)',
    borderRadius: 10,
    borderWidth: 2,
    borderColor: '#FFF',
    overflow: 'hidden',
    zIndex: 10,
    justifyContent: 'center',
    alignItems: 'center'
  },
  nitroBarFill: {
    position: 'absolute',
    left: 0,
    top: 0,
    bottom: 0,
  },
  nitroBarText: {
    color: '#FFF',
    fontWeight: 'bold',
    fontSize: 10,
    fontStyle: 'italic',
    zIndex: 2,
  },

  /* ESTILOS DOS BOTÕES DE DIREÇÃO/NITRO */
  jumpArea: {
    position: 'absolute',
    backgroundColor: '#fff',
    left: 40,
    bottom: 30,
    height: 90,
    width: 90,
    borderRadius: 45,
    zIndex: 30,
    elevation: 5,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.5,
    shadowRadius: 5,
  },
  drivingControls: {
    position: 'absolute',
    bottom: 30,
    right: 40,
    flexDirection: 'row',
    alignItems: 'center',
    gap: 20,
    zIndex: 30,
  },
  throttleBtn: {
    width: 90,
    height: 90,
    borderRadius: 45,
    backgroundColor: 'rgba(0, 208, 132, 0.8)',
    borderWidth: 4,
    borderColor: '#FFF',
    justifyContent: 'center',
    alignItems: 'center',
    elevation: 5,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.5,
    shadowRadius: 5,
  },
  throttleBtnPressed: {
    backgroundColor: 'rgba(0, 208, 132, 1)',
    transform: [{ scale: 0.95 }],
  },
  throttleBtnText: {
    color: '#FFF',
    fontWeight: '900',
    fontSize: 14,
    fontStyle: 'italic'
  },
  nitroBtn: {
    width: 70,
    height: 70,
    borderRadius: 35,
    backgroundColor: 'rgba(0, 255, 255, 0.9)',
    borderWidth: 3,
    borderColor: '#FFF',
    justifyContent: 'center',
    alignItems: 'center',
    elevation: 5,
  },
  nitroBtnText: {
    color: '#000',
    fontWeight: '900',
    fontSize: 14,
    fontStyle: 'italic'
  },

  block: {
    position: 'absolute',
    height: 2000,
    backgroundColor: '#00D084',
    borderTopWidth: 5,
    borderColor: '#FFF',
    zIndex: 3
  },
  overlay: {
    ...StyleSheet.absoluteFillObject,
    justifyContent: 'center',
    alignItems: 'center',
    zIndex: 20
  },
  titleText: {
    fontSize: 48,
    fontWeight: '900',
    color: '#FFD700',
    textShadowColor: '#FF4500',
    textShadowOffset: { width: 3, height: 3 },
    textShadowRadius: 5
  },
  countdownText: {
    fontSize: 72,
    fontWeight: '900',
    color: '#00D084',
    textShadowColor: '#FFF',
    textShadowOffset: { width: 2, height: 2 },
    textShadowRadius: 10,
    fontStyle: 'italic'
  },
  blindEffect: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: 'rgba(0, 0, 0, 0.85)',
    zIndex: 15,
  },
});