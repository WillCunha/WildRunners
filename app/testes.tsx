...
export default function Mapa() {
  ...
  type CardEffect = 'swap' | 'chains' | 'heavy_gravity' | 'invert_controls' | 'blind' | 'panic' | 'ghost' | 'score_boost';
  ...
  useEffect(() => {
    ...
    const loop = setInterval(() => {
      ...
      // --- 6. EFEITO DA CORRENTA (CHAINS) ---

      if (activeChainsState && activeChainsState.duration > 0) {
        const { callerId, targetId } = activeChainsState;
        ...
        if (callerId === 'player') {
          ...
          if (callerBot) callerBot.speed += PULL_FORCE;
        }

        if (targetId === 'player') {
          playerSpeed.current -= PULL_FORCE;
          if (playerSpeed.current < 1) playerSpeed.current = 1;
        } else {
          const targetBot = botsRef.current.find(b => b.id === targetId);
          if (targetBot) {
            targetBot.speed -= PULL_FORCE;
            if (targetBot.speed < 1) targetBot.speed = 1;
          }
        }

        setActiveChainsState(prev => prev ? { ...prev, duration: prev.duration - 1 } : null);
      } else if (activeChainsState && activeChainsState.duration <= 0) {
        setActiveChainsState(null);
      }

      // --- 7. COLISÕES DO PLAYER (FÍSICA CORRIGIDA) ---
        ...
        {activeSwap && (
          ...
          <ChainsEffect
            ...
            onChainsExecute={(targetId) => {
              applyCardEffect('chains', targetId, 'player');
